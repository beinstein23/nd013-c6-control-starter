...
// 	pid_steer.Init(0.27, 0.00099,0.9, 1.2, -1.2);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          int min_idx;
          // compute the disired yaw
          for (int i = 0; i< x_points.size(); ++i) {
         	 double metric = std::pow((x_position - x_points[i]), 2) + std::pow((y_position - y_points[i]),2);
          ...
          **/
          ...
          double error_steer = angle_between_points(x_position,y_position,x_points[min_idx],y_points[min_idx]) - yaw;
           ...
           std::cout<<"current position: x="<<x_position<<"; y ="<<y_position<<std::endl;
          ...
          **/
          ...
          error_throttle = v_points[min_idx] - velocity;
           std::cout<<"error_throttle:"<<error_throttle<<std::endl;
				std::cout<<"planned vel:"<<v_points[min_idx]<<std::endl;
				std::cout<<"currrent vel:"<<velocity<<std::endl;
